...
private:
  ...
  void scan_cb(const sensor_msgs::msg::LaserScan::SharedPtr scan_msg)
  {
    ...
    //?pcl::PointCloud<pcl::PointXYZ> cloud_pc;
    ...
    seg.setDistanceThreshold(0.05); // Adjust based on point cloud
    ...
    while(true){
        ...
        line_coeffs.push_back(line_coefficients);
    ...
    std::cout << "number of lines found is: " << line_coeffs.size() << std::endl;
    ...
    */
    ...
    // Implement a simple 2D intersection calculation using line coefficients here

    findCrossShape(line_coeffs);
  ...
  void findCrossShape(const std::vector<pcl::ModelCoefficients::Ptr> &line_coeffs)
  {
    // Define a target angle for "cross" detection (e.g., 90 degrees, in radians)
    const float target_angle = pcl::deg2rad(90.0f); 
    const float angle_tolerance = pcl::deg2rad(10.0f); // Acceptable tolerance range
    ...
    for (size_t i = 0; i < line_coeffs.size(); ++i)
    {
      for (size_t j = i + 1; j < line_coeffs.size(); ++j)
      {
        ...
        Eigen::Vector2f dir_i(line_coeffs[i]->values[3], line_coeffs[i]->values[4]);
        Eigen::Vector2f dir_j(line_coeffs[j]->values[3], line_coeffs[j]->values[4]);
        ...
        if (std::abs(angle - target_angle) < angle_tolerance)
        {
          ...
          publishCrossMarker(line_coeffs[i], line_coeffs[j]);
          return;
        }
      }
    }
  ...
  void publishCrossMarker(const pcl::ModelCoefficients::Ptr &line1, const pcl::ModelCoefficients::Ptr &line2)
  {
    // Create markers for visualization in Rviz
    visualization_msgs::msg::Marker marker1, marker2;

    marker1.header.frame_id = marker2.header.frame_id = "lidar_frame";
    ...
    marker1.scale.x = marker2.scale.x = 0.05; // Set line width
    ...
    p1_start.x = line1->values[0]; p1_start.y = line1->values[1]; p1_start.z = line1->values[2];
    p1_end.x = p1_start.x + line1->values[3]; 
    p1_end.y = p1_start.y + line1->values[4]; 

    p2_start.x = line2->values[0]; p2_start.y = line2->values[1]; p2_start.z = line2->values[2];
